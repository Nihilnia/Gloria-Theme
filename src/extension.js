...
const messages = {
	...
	ERROR_WORKBENCH_NOT_FOUND: "Neon Dreams could not find the workbench HTML file. This is likely due to a change in VS Code's internal structure. Please open an issue on the Gloria Theme GitHub repository to report this.",
	...
	let disposable = vscode.commands.registerCommand('gloria.enableNeon', function () {
		...
		try {
			...
			if (!isEnabled) {
				...
				// add script tag
				output = html
					.replace(/\<\/html\>/g, `	<!-- GLORIA --><script src="neondreams.js"></script><!-- NEON DREAMS -->\n`);
				output += '</html>';
...
function uninstall() {
	...
	try {
		...
		if (isEnabled) {
			// delete gloria script tag if there
			let output = html.replace(/^.*(<!-- GLORIA --><script src="neondreams.js"><\/script><!-- NEON DREAMS -->).*\n?/mg, '');
			fs.writeFileSync(htmlFile, output, "utf-8");
